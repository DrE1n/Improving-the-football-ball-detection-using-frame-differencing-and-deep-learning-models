...
BALL_CLASS_ID = 0
IMG_DIR = "D:\dataset_new_split\images"
LBL_DIR = "D:\dataset_new_split\labels"
...
for i in range(len(img_files) - 1):
    ...
    if pts1 is not None:
        ...
        if len(good1_xy) > 20: # find the best transformation to map the points from frame 2 back to frame 1
            ...
            if M is not None:
                ...
                img2 = cv2.warpAffine(img2, M, (img2.shape[1], img2.shape[0]))  # apply transform to the picture, cut the last channels dimension from img2
                gray2 = cv2.cvtColor(img2, cv2.COLOR_BGR2GRAY).astype(np.float32)  # recompute grayscale, convert back to float32
    ...
    cv2.imwrite(os.path.join("dataset_diffstab/images/train", img_name), gray_diff)
    cv2.imwrite(os.path.join("dataset_diffstab/bounding_box/train", img_name), bbox_vis)

    with open(os.path.join("dataset_diffstab/labels/train", lbl_name), "w") as f:
